...
/* IWDG: Independent Watchdog (on APB1, but clocked by LSI ~40kHz,
 * independent of the main system clock and unaffected by it) */
#define IWDG_BASE          (APB1PERIPH_BASE + 0x3000UL)/* 0x40003000 */
 ...
 *   Independent Watchdog. Clocked by LSI (~40 kHz), runs even
 *   if the main clock fails. Once started it CANNOT be stopped
 *   (only a reset clears it).
 * ============================================================ */
typedef struct {
    __IO uint32_t KR;     /* 0x00: Key Register   (write-only commands) */
    __IO uint32_t PR;     /* 0x04: Prescaler Register                   */
    __IO uint32_t RLR;    /* 0x08: Reload Register                      */
    __IO uint32_t SR;     /* 0x0C: Status Register (update flags)       */
} IWDG_TypeDef;

/* ---- IWDG_KR key values (write to KR to issue a command) ---- */
#define IWDG_KEY_RELOAD   0xAAAAU  /* refresh the counter (prevents reset) */
#define IWDG_KEY_ENABLE   0xCCCCU  /* start the watchdog                   */
#define IWDG_KEY_UNLOCK   0x5555U  /* unlock PR and RLR for writing        */

/* ---- IWDG_SR bits (1 = the matching register write is pending) ---- */
